...
Every card, character, and piece of the cabin in this game is **drawn by code**. There are no
hand-painted files and no AI-generated images: [`generate_art.py`](generate_art.py) draws all of it
from math and logic, and running it twice produces pixel-identical output. This file is the spec
that script follows — the rules that keep a few thousand lines of drawing code looking like one
cohesive room.

**Start with [The unit](#the-unit-art-pixels-au-and-integer-scale--read-first).** Everything else
depends on it.

| | |
|---|---|
| Draw the art | `pip install -r requirements.txt && python3 generate_art.py` |
| Check it | `npm run art:check` (deterministic, card art unchanged, no holes in sprites) |
| Output | `public/art/` — **never edit those PNGs by hand**; edit the script and regenerate |
| Sounds | [`generate_audio.py`](generate_audio.py), synthesized from waveform math, no samples |

### Contents

- [The unit: art pixels and integer scale](#the-unit-art-pixels-au-and-integer-scale--read-first) — the one rule everything obeys
- [Palette](#palette--cabin-by-the-fire) — the colours, and why the cards and the room have separate ones
- [Cards](#cards) — the 24 faces, frozen once drawn
- [Environment shading](#environment-shading--ramp-phase-2d) and [lighting](#lighting--light_from-phase-2d5)
- [Tiles vs placed objects](#tiles-vs-placed-objects--different-rules) — repeating surfaces have their own rules
- [Scene objects](#scene-objects-phase-2d2), [living things](#living-things-phase-2d3), [furniture](#wall-and-floor-furniture-phase-2g4)
- [Animation](#animation-convention), [output](#output), [audio](#audio-phase-2c)

Headings tagged like *(Phase 2d.2)* name the build stage a section came from. They are history, not
structure; ignore them unless you are reading the commit log alongside.

Assets sit on a native pixel grid and display at a whole-number scale with
...
stingers only, deliberately no music: a card game in a quiet cabin does not want a soundtrack.
